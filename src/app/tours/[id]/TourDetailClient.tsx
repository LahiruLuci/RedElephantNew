...
// import styles from './page.module.css'; // We'll keep using the same styles or inline if needed
...
function SectionLabel({ children, accent }: { children: React.ReactNode; accent: string }) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.1rem,3vw,1.8rem)', fontWeight: 800, color: '#1A1714', margin: 0, letterSpacing: '-.01em' }}>{children}</h2>
        </div>
    );
}
...
function IncludedItem({ text, isIncluded, accent, isMobile, index, vis }: { text: string; isIncluded: boolean; accent: string; isMobile: boolean; index: number; vis: boolean }) {
    ...
    return (
        <div
            onMouseEnter={() => setHover(false)} // disabled hover scale for now or fix
...
export default function TourDetailClient({ pkg }: { pkg: any }) {
    ...
    return (
        <div style={{ width: '100%', maxWidth: '100vw', position: 'relative', boxSizing: 'border-box' }}>
                ...
                <div style={{ maxWidth: 1440, margin: '0 auto', padding: isMobile ? '0 16px 16px' : '0', boxSizing: 'border-box' }}>
                    <div style={{
                        ...
                        gridTemplateColumns: isMobile ? '1fr' : (
                            (pkg.galleryImages.slice(0, 3).findIndex((img: string) => (selectedHeroImg || pkg.heroImage) === img) === 1) ? '1fr 2fr 1fr' :
                            (pkg.galleryImages.slice(0, 3).findIndex((img: string) => (selectedHeroImg || pkg.heroImage) === img) === 2) ? '1fr 1fr 2fr' :
                            '2fr 1fr 1fr'
                        ),
                        gap: 3,
            ...
            <BodySection pkg={pkg} isMobile={isMobile}
